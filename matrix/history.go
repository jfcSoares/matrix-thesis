// Based on https://github.com/tulir/gomuks/blob/master/matrix/history.go
package matrix
...
func (hm *HistoryManager) Get(room *rooms.Room, eventID id.EventID) (evt *mxevents.Event, err error) {
...
func (hm *HistoryManager) Update(room *rooms.Room, eventID id.EventID, update func(evt *mxevents.Event) error) error {
...
func (hm *HistoryManager) Append(room *rooms.Room, events []*event.Event) ([]*mxevents.Event, error) {
	evts, _, err := hm.store(room, events, true)
	return evts, err
}

func (hm *HistoryManager) Prepend(room *rooms.Room, events []*event.Event) ([]*mxevents.Event, uint64, error) {
	return hm.store(room, events, false)
}

func (hm *HistoryManager) store(room *rooms.Room, events []*event.Event, append bool) (newEvents []*mxevents.Event, newPtrStart uint64, err error) {
	hm.Lock()
	defer hm.Unlock()
	newEvents = make([]*mxevents.Event, len(events))
...
func (hm *HistoryManager) Load(room *rooms.Room, num int, ptrStart uint64) (events []*mxevents.Event, newPtrStart uint64, err error) {
	hm.Lock()
	defer hm.Unlock()
	err = hm.db.View(func(tx *bolt.Tx) error {
...
func stripRaw(evt *mxevents.Event) {
	evtCopy := *evt.Event
...
func marshalEvent(evt *mxevents.Event) ([]byte, error) {
	stripRaw(evt)
...
func unmarshalEvent(data []byte) (*mxevents.Event, error) {
	evt := &mxevents.Event{}
...
func put(streams, eventIDs *bolt.Bucket, evt *mxevents.Event, key uint64) error {
